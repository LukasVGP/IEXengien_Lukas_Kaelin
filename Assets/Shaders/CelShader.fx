// Cel Shader that uses cel shading effect on objects
...
float4 PS(VS_OUTPUT input) : SV_Target
{
    ...
    // Emissive
    float edgeThickness = 0.84f;
    float edgeThreshold = 0.01f;
    float e = 1.0f - saturate(dot(view, n));
    e = smoothstep(edgeThickness - 0.1f, edgeThickness + 0.1f, e); // Edge detection
    ...
    d = smoothstep(0.005f, 0.1f, d); // Cel shading step
    ...
    s = smoothstep(0.005f, 0.1f, s); // Cel shading step
